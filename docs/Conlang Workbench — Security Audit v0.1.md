...
Review of command and mutation boundaries is complete for the current
TypeScript source and persistence architecture.

The review completed thirteen substantial authority boundaries:
...
- cross-family settings transaction concurrency and settled-authority ordering

SEC-006-H1 through SEC-006-H13 are remediated and verified.

The final H13 review established a common plugin-wide settings-authority
serialization boundary for overlapping settings-backed transactions while
preserving the specialized validation, filesystem, persistence, reload, and
rollback semantics established by H1 through H12.
...
#### SEC-006-H4 — Plugin-initiated configuration deletion lacked exact-target confirmation and fail-safe mutation handling

**Severity:** Hardening

Several settings controls could delete persisted plugin configuration
immediately from their current rendered array position:

- removing a language configuration;
- deleting a cypher sheet and its contained rules;
- deleting an individual cypher rule;
- deleting an individual inflection rule.

These operations did not modify or delete vault Markdown files, but they could
remove creator-configured Workbench state without a shared explicit
authorization boundary. Because settings rendering and confirmation are
asynchronous UI concerns, a rendered numeric array index must also not become
authority to delete whatever object later occupies that position.

The required invariant is:

> Confirmation authorizes deletion of the exact object shown to the user, not
> whatever later occupies its former array index.

**Remediation:**

`delete-confirm-modal.ts` now provides one shared fail-closed authorization
boundary for plugin-initiated deletion. The modal owns authorization only and
has no settings or vault mutation authority.

Only the explicit destructive button resolves approval. Cancel, Escape, outside
click, the modal close control, or another implicit close resolves cancellation.
A decision guard prevents the confirmation promise from being resolved more
than once.

Caller-supplied names, labels, rule text, and messages are rendered through DOM
text APIs rather than HTML interpretation.

Each deletion caller captures the object that was actually presented for
authorization. After the asynchronous confirmation returns, the caller
re-finds that same object by identity in the current array before mutating
settings. A stale rendered numeric index therefore cannot authorize deletion of
a different object.

The confirmation text describes the actual impact:

- language removal states that the Workbench language configuration will be
  removed while its configured vault folders and files will not be deleted;
- cypher-sheet deletion identifies the sheet and reports how many contained
  rules will be removed from settings;
- cypher-rule deletion identifies the input and output when available;
- inflection-rule deletion identifies its label when available.

If persistence fails after a cypher sheet, cypher rule, or inflection rule is
removed in memory, the exact removed object is restored to the in-memory
configuration.

Language removal additionally snapshots the previous language list,
active-language references, primary-language reference, and relevant expanded
settings-card state. If the initial settings save fails, that state is restored
in memory. If removal is successfully persisted but the subsequent language
reload is blocked by canonical-source preflight, Workbench restores the previous
configuration and persists the rollback. The blocked preflight itself leaves
the currently loaded runtime language data untouched.

This remediation does not claim transactional recovery from an arbitrary
exception that occurs after source preflight while runtime inventories are
being rebuilt. That broader reload-failure boundary remains separate from this
deletion-authorization finding.

**Verification:**

`scripts/test-delete-confirmation.mjs` bundles the production
`delete-confirm-modal.ts` against a minimal Obsidian modal test double and
verifies:

- Cancel resolves false;
- explicit destructive confirmation resolves true;
- implicit close resolves false;
- a confirmation resolves only once;
- creator-controlled title and message strings remain literal text.

The existing language-source-preflight regression suite also passes.

Manual Obsidian runtime verification confirmed:

- cancelling deletion of the existing `"the" → "ka"` cypher rule preserved it;
- cancelling deletion of the `Common Words` cypher sheet preserved the sheet
  and its three rules;
- cancelling deletion of the `plural` inflection rule preserved it;
- confirming deletion of a disposable cypher rule removed only that rule;
- confirming deletion of a disposable cypher sheet removed that sheet and its
  contained disposable rule;
- confirming deletion of a disposable inflection rule removed only that rule;
- cancelling removal of the disposable `Language 3` configuration preserved it;
- confirming removal of `Language 3` removed the configuration while the
  remaining Test Language primary state and Mer inactive state remained
  intact.

The disposable `Language 3` configuration had no corresponding vault source
folder, so that runtime case did not independently exercise preservation of an
existing language folder. The implementation contains no vault-file or
vault-folder deletion operation in this removal path, and the confirmation
explicitly communicates that configured vault folders and files are retained.

The runtime exercise also exposed a separate usability issue: settings
rerenders after add/delete operations can move the user back to the top of the
settings page. Scroll-position preservation is follow-up usability work and is
not part of this deletion-authority remediation.

The production build succeeds. The deletion-confirmation and language-source
preflight regression suites pass. Lint reports zero errors with the established
...
#### SEC-006-H13 — Overlapping settings transactions could derive rollback authority from provisional state

**Severity:** Hardening

Follow-up review after H12 demonstrated that independently initiated
settings-backed transactions could overlap while sharing the same mutable
complete-settings object. A later transaction could therefore read provisional
state installed by an earlier in-flight transaction and incorrectly adopt that
state as its rollback authority.

The reproduced failure required no assumption about undocumented Obsidian
`saveData()` snapshot or completion-order semantics: two failed overlapping
ordinary-setting transactions were sufficient to leave an unpersisted
intermediate value authoritative in memory.

The required invariant is:

> A settings-authority transaction must derive rollback state only from settled
> authority. Provisional state belonging to another in-flight transaction must
> never become rollback authority.

**Remediation:**

`SettingsAuthorityQueue` now provides one plugin-wide outer serialization
boundary for settings-backed authority transactions. Callers enter the queue
before reading prior authority, constructing candidates from mutable settings,
or installing provisional state, and retain it through the complete logical
transaction, including persistence, runtime reload, and any authorized rollback
or compensating persistence.

The common boundary now covers ordinary persisted settings, primary and active
language configuration, case-sensitive matching, linguistic rules, canonical
language sources, Language Profile paths, language-root repair, language rename,
language membership, language creation, language removal, runtime reload
coordination, and welcome-state persistence.

Specialized H1-H12 transaction modules retain their established validation,
filesystem, persistence, reload, and rollback semantics. Linguistic-rule
transactions retain their dedicated queue inside the common settings queue with
the documented acquisition order
`settingsAuthorityQueue -> linguisticRuleStateQueue`.

Runtime reloads initiated outside an already-queued settings transaction now
use `reloadSettledLanguageState()` so runtime indexes cannot rebuild from
another transaction's provisional configuration.

The welcome-state persistence path is also executed inside the common queue
with failed-save rollback semantics rather than remaining a fire-and-forget
whole-settings write.

**Verification:**

Focused H13 regression coverage verifies overlapping transaction ordering,
queue recovery after rejection, cross-family serialization, runtime reload
coordination, language creation and removal boundaries, and welcome-state
persistence behavior. The production build, lint baseline, formatting checks,
and `git diff --check` passed after remediation.

The full investigation, transaction inventory, lock ordering, architectural
rationale, and verification record are preserved in
`docs/Conlang Workbench — Settings Transaction Concurrency Remediation.md`.

**Status:** Remediated and verified.

### Status

**Pass** — The current command and mutating-operation surfaces have been
inventoried against the TypeScript source and reviewed through SEC-006-H13.
Vault-content mutation, settings/configuration mutation, persistence failure,
runtime reload, filesystem authority, destructive confirmation, and
cross-family asynchronous settings authority now have explicit fail-closed or
transactional boundaries appropriate to their operation. SEC-006-H1 through
SEC-006-H13 are remediated and verified.
...
| SEC-006-H13 | §6 Commands and Mutating Operations | Remediated and verified          | Hardening | Overlapping settings-backed transactions could read provisional state belonging to another in-flight transaction and incorrectly adopt that state as rollback authority.                                                                                              | Code review; focused H13 settings-authority-queue and cross-family regression coverage; runtime reload, language creation/removal, and welcome-persistence coverage; production build; lint baseline; formatting checks; `git diff --check`          | One plugin-wide `SettingsAuthorityQueue` now serializes complete settings-authority transactions before authority-sensitive reads or provisional mutation and retains authority through persistence, reload, and authorized rollback or compensating persistence. Specialized H1-H12 transaction semantics remain intact, linguistic-rule locking has an explicit acquisition order, external runtime reloads wait for settled settings authority, and welcome-state persistence shares the same boundary. |
